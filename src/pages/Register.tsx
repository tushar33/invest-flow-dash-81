...
import { User, Mail, Phone, Lock, Eye, EyeOff, ShieldCheck, TrendingUp, Coins, MapPin } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import trinityLogo from "@/assets/trinity-arrows-logo.png";
import { LANG } from "@/lib/language";

const REGISTER_FEATURE_ICONS = [Coins, TrendingUp, ShieldCheck] as const;
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          <div className="flex items-center gap-1.5">
            <div className="h-20 w-20 flex items-center justify-center">
              <img src={trinityLogo} alt={LANG.brand.name} className="h-full w-full object-contain drop-shadow-[0_0_16px_hsl(var(--accent)/0.5)]" />
            </div>
            <div className="leading-tight">
              <span className="block text-xl font-bold tracking-tight">{LANG.brand.name}</span>
              <span className="block text-[10px] uppercase tracking-[0.2em] text-primary-foreground/60">{LANG.brand.tagline}</span>
            </div>
          </div>

          <div className="space-y-6 max-w-md">
            <h2 className="text-4xl font-bold leading-tight tracking-tight">
              {LANG.auth.registerHeroTitle}
            </h2>
            <p className="text-base text-primary-foreground/70 leading-relaxed">
              {LANG.auth.registerHeroDescription}
            </p>

            <div className="grid grid-cols-1 gap-3 pt-4">
              {LANG.auth.registerFeatures.map((f, i) => {
                const Icon = REGISTER_FEATURE_ICONS[i];
                return (
          ...
          <p className="text-xs text-primary-foreground/50">{LANG.brand.copyright(new Date().getFullYear())}</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-12 relative">
        ...
        <div className="w-full max-w-sm space-y-6 relative z-10">
          <div className="text-center lg:text-left">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-foreground lg:text-3xl">{LANG.auth.createAccount}</h1>
            <p className="text-sm text-muted-foreground mt-1">{LANG.auth.registerSubtitle}</p>
          </div>

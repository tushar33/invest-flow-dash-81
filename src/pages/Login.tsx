...
import { User, Lock, Eye, EyeOff, ShieldCheck, TrendingUp, Coins } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import trinityLogo from "@/assets/trinity-arrows-logo.png";
import { LANG } from "@/lib/language";

const LOGIN_FEATURE_ICONS = [Coins, TrendingUp, ShieldCheck] as const;
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div className="space-y-6 max-w-md">
            <h2 className="text-4xl font-bold leading-tight tracking-tight">
              {LANG.auth.loginHeroTitle}
            </h2>
            <p className="text-base text-primary-foreground/70 leading-relaxed">
              {LANG.auth.loginHeroDescription}
            </p>

            <div className="grid grid-cols-1 gap-3 pt-4">
              {LANG.auth.loginFeatures.map((f, i) => {
                const Icon = LOGIN_FEATURE_ICONS[i];
                return (
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-12 relative">
        ...
        <div className="w-full max-w-sm space-y-6 relative z-10">
          <div className="text-center lg:text-left">
            ...
            <p className="text-sm text-muted-foreground mt-1">{LANG.auth.signInPrompt}</p>
          </div>
